// 本文件用于wechat 3rd party SuiteAPI，基础文件，主要用于Token的处理和mixin机制
 ...
 * 
 * ```
 * var api = new SuiteAPI('suite_id', 'suite_secret', function* () {
 *   // 传入一个获取全局token的方法
 ...
 */
var SuiteAPI = function (suite_id, suite_secret, getToken, saveToken) {
 ...
 */
SuiteAPI.prototype.setOpts = function (opts) {
  this.defaults = opts;
};
 ...
 * 设置urllib的hook
 */
SuiteAPI.prototype.request = function* (url, opts) {
  var options = {};
...
/*!
 * 根据创建SuiteAPI时传入的appid和suite_secret获取access token
 * 进行后续所有SuiteAPI调用时，需要先获取access token
 * 详细请看：<http://mp.weixin.qq.com/wiki/index.php?title=获取access_token> * 应用开发者无需直接调用本SuiteAPI。 * Examples:
 * ```
 ...
 * ```
 */
SuiteAPI.prototype.getAccessToken = function* () {
  var url = this.prefix + 'gettoken?suite_id=' + this.suite_id + '&suite_secret=' + this.suite_secret;
 ...
 * 应用开发者无需直接调用此SuiteAPI。
 * Examples:
 * ```
 * yield* api.ensureAccessToken();
 * ```
 */
SuiteAPI.prototype.ensureAccessToken = function* () {
...
/**
 * 用于支持对象合并。将对象合并到SuiteAPI.prototype上，使得能够支持扩展
 * Examples:
 * ```
 * // 媒体管理（上传、下载）
 * SuiteAPI.mixin(require('./lib/api_media'));
 * ```
 * @param {Object} obj 要合并的对象
 */
SuiteAPI.mixin = function (obj) {
  for (var key in obj) {
    if (SuiteAPI.prototype.hasOwnProperty(key)) {
      throw new Error('Don\'t allow override existed prototype method. method: '+ key);
    }
    SuiteAPI.prototype[key] = obj[key];
  }
};

SuiteAPI.AccessToken = AccessToken;
